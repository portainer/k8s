...
main() {
  ...
  curl -L https://portainer.github.io/k8s/deploy/manifests/agent/portainer-ce213-agent-edge-k8s.yaml -o portainer-agent-edge-k8s.yaml || errorAndExit "Unable to download agent manifest"

  info "Creating Portainer namespace..."
  kubectl create namespace portainer

  info "Creating agent configuration..."
  configmapCmd="kubectl create configmap portainer-agent-edge"
  configmapCmd+=" --from-literal="edge.id=$EDGE_ID""
  configmapCmd+=" --from-literal="edge.insecure_poll=$EDGE_INSECURE_POLL""
  if [ ! -z $EDGE_SECRET ]; then
    configmapCmd+=" --from-literal="edge.secret=$EDGE_SECRET""
  fi
  configmapCmd+=" -n portainer"
  $configmapCmd
